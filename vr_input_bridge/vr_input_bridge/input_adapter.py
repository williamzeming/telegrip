"""ROS 2 adapter that turns controller input streams into latched teleop commands."""
...
class TeleopInputAdapter(Node):
    ...
    def __init__(
        ...
        self.get_logger().info(
            "Teleop input adapter started: %s -> %s at %.1f Hz",
            self.input_prefix,
            self.output_prefix,
            publish_rate_hz,
        )
...
def parse_args():
    ...
    parser.add_argument("--publish-rate", type=float, default=30.0, help="Command republish rate in Hz")
    parser.add_argument(
        "--output-frame-id",
        default="",
